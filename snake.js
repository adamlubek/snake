...
const createElem = col => {
  ...
  elem.style['background-color'] = col === empty ? 'silver' : (col === snake ? 'cornflowerblue' : 'red');
...
const handlers = {
  'ArrowUp': [() => setDir(-1, 0)],
  'ArrowDown': [() => setDir(1, 0)],
  'ArrowLeft': [() => setDir(0, -1)],
  'ArrowRight': [() => setDir(0, 1)]
}
document.addEventListener("keydown", e => handlers[e.code].forEach(action => action()));
